	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if report_type=="SUSPENCE":
				return HttpResponseRedirect(reverse('TCC11_12.automation.views.suspence_calculation'))
			elif report_type=="CDF" and type_of_consultancy=="CDF":
				return HttpResponseRedirect(reverse('TCC11_12.automation.views.cdf_calculation'))
			else :
				return HttpResponseRedirect(reverse('TCC11_12.automation.views.report_calculation'))
...
def suspence_calculation(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			p = Amount(job_no = job_no ,date=datetime.date.today(),total=total,lab=lab,tds=tds, net_total=net_total, balance=balance, development_fund=development_fund, 
	...
	if request.method == 'POST':
		...
		if form1.is_valid():
			...
			s.save()
                        p = Amount(job_no = job_no ,type = type,date=datetime.date.today(),total=total,lab=lab,tds=tds, net_total=net_total, balance=balance, development_fund=development_fund, 
			service_tax=service_tax, higher_education_tax=higher_education_tax, education_tax=education_tax, field=field, 
			college_income = college_income, admin_charge=admin_charge, consultancy_asst=consultancy_asst, other_field=other_field,report_type=report_type)
			p.save()
...
def report_calculation(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			receipt_no = receipt_no['receipt_no__max'] 
			ClientJob.objects.filter(job_no = client.job_no).update( receipt_no = receipt_no)
			p = Amount(job_no = job_no ,date=datetime.date.today(),total=total,type=type,lab=lab,tds=tds, net_total=net_total, balance=balance, development_fund=development_fund, 
			service_tax=service_tax, higher_education_tax=higher_education_tax, education_tax=education_tax, field=field, 
			college_income = college_income, admin_charge=admin_charge, consultancy_asst=consultancy_asst, other_field=other_field,report_type=report_type)
			p.save()
		...
		if form.is_valid():
			...
			cd = form.cleaned_data
			        ...
			        sign = get_object_or_404(Variable, pk='3')
                                from TCC11_12.automation.choices import *
			        client = ClientJob.objects.filter(date__range=(start_date,end_date))
			        amount = Amount.objects.filter(date__range=(start_date,end_date)).filter(Q(type='CHEQUE')|Q(type='DD')| Q(type='ONLINE'))
			        retrieve()
                                suspence = Suspence.objects.all()
                                balance_temp = Amount.objects.filter(date__range=(start_date,end_date)).filter(Q(type='CHEQUE')| 
 Q(type='DD')|Q(type='ONLINE')).aggregate(Sum('balance'))
			        balance= balance_temp['balance__sum']
			        tds_temp = Amount.objects.filter(date__range=(start_date,end_date)).filter(Q(type='CHEQUE')|Q(type='DD')|Q(type='ONLINE')).aggregate(Sum('tds'))
			        tds= tds_temp['tds__sum']
			        template ={'form':form,'tds':tds, 'title':title ,'sign':sign, 'date': start_date, 'balance':balance, 'client':client,'amount':amount,'type':type,'suspence':suspence}
